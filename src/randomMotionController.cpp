...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "randomMotionController");
...
// Use publish() function to send message objects
  cmd.linear.x = 0.25; // 0.25 gives a pretty slow motion //initialize forward velocity
  cmd.angular.z = 0.0; //initialize turning velocity (assume positive z axis points up)

  // All other velocity commands generally zero
  cmd.linear.y = 0.0;
  cmd.linear.z = 0.0;
  cmd.angular.x = 0.0;
  cmd.angular.y = 0.0;
...
// Perform Calculation and assign message object data values
  ...
  //FOR TROUBLESHOOTING - constant values of
  cmd.linear.x = 0.01;
  cmd.linear.y = -0.25;
  cmd.linear.z = 0.0;
  cmd.angular.z = 0.025;
